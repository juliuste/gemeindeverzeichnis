...
tape('gemeindeverzeichnis', (t) => {
	...
	.on('data', (item) => {
		...
		if(isRegierungsbezirkMuenster(item)) t.pass('contains regierungsbezirk `Münster`')
		if(isRegionStuttgart(item)) t.pass('contains region `Stuttgart`')
		if(isKreisGelsenkirchen(item)) t.pass('contains kreis `Gelsenkirchen`')
		if(isGemeindeverbandMarl(item)) t.pass('contains gemeindeverband `Marl`')
		if(isGemeindePrezelle(item)) t.pass('contains gemeinde `Prezelle`')
	})
})